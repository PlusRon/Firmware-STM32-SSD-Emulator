...
## 五、結論
- **解決當機問題** : 當系統當掉時，你必須看懂 **Map file（連結器產生的報告）**，對照 Linker Script 找問題
- **節省成本** : 透過 `-nostdlib` 和 `–gc-sections` 把程式壓小，讓公司換成更便宜（Flash 更小）的晶片，省下數百萬成本
- **自動化生產** : Makefile 可以直接丟上伺服器做 **自動測試（CI/CD）**，不需要依賴 Keil 的圖形介面
