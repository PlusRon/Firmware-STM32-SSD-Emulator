...
|GPIOA|MODER|引腳模式切換|Bits (PA12)|`10` : 設定為 Alternate Function (複用模式) 以支援 **RTS**|
|GPIOA|AFRH|複用功能選擇|Bits (PA12)|`0001` : 指定為 AF1，將引腳硬體連通至 **USART1_RTS**|
|GPIOC|MODER|引腳模式切換|Bits (PC6)|`01`: 設定為 General Purpose Output (一般輸出模式)|
...
|USART1|CR1|IDLE|中斷致能|Bit 4 (IDLEIE)|`1`: 當偵測到線路空閒超過 1 Byte 時間，觸發 CPU 中斷|
...
|SysTick|CTRL|SysTick 總控制|Bit 2 (CLKSOURCE)<br>Bit 1 (TICKINT)<br>Bit 0 (ENABLE)|`7` (二進制 `111`): 使用處理器時鐘、啟動中斷、啟動計時器|
