...
- #### 各周邊、暫存器、位元 (Bit) 控制
  ...
  |:---|:---|:---|:---|:---|
  |RCC|AHBENR|GPIO周邊<br>供電開關|Bit 17 (IOPAEN)|`1`: 啟動 GPIOA 時脈，GIPOA 周邊暫存器才能運作 (MODER、ODR、AFRH、AFRL 暫存器才能通電做控制)|
  |RCC|APB2ENR|UART周邊<br>供電開關|Bit 14 (USART1EN)|`1`: 啟動 USART1 時脈，才能控制 UART1 周邊(通訊電路開始運作)|
  ...
  |USART1|CR1|UART1模組<br>總控制|Bit 0 (UE)、Bit 3 (TE)、Bit 2 (RE)|Bit-0 = `1`: 啟動 UART1 模組總開關<br>Bit-3 = `1`: 啟動發送器 (Transmitter)<br>Bit-2 = `1`: 啟動接收器 (Receiver)|
  |USART1|ISR|狀態監控<br>(**唯讀**)|Bit 7 (TXE)、Bit 5 (RXNE)|Bit-7 = `1`: 發送區空 (**可寫資料**)<br>Bit-5 = `1`: 接收區有資料 (**可讀資料**)|
  |USART1|TDR|發送資料<br>暫存器|Bits [8:0]|寫入此處 ： 資料由 TX 腳位發出去|
  |USART1|RDR|接收資料<br>暫存器|Bits [8:0]|讀取此處 ： 由 RX 腳位接收資料|
