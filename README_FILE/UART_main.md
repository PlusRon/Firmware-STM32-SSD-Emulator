...
- **非同步通訊 (UART)** : 雙方沒有共同的時脈，因此必須事先約定好 **Baud Rate (波特率)**
    ```
    同步通訊 (SPI/I2C) : 由一根專門的 Clock 線告訴接收端「現在該讀取資料了」
    ```
- **UART** 與 I2C 或 SPI 最大的不同在於它 **沒有時脈線 (Clock Signal)**
- **工作原理** : 雙方各自使用內部計時器，約定**每秒鐘取樣幾次（例如 115200 次）**，若頻率不一致，資料就會因偏移而解讀成亂碼
...
### 3. 關鍵參數與常見配置
- 在開發過程中（如 **Minicom 設定**），會看到 **115200 8N1**
