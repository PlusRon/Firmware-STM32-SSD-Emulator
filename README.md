...
- #### [startup.c 實作](README_FILE/startup.md)
  ...
  - **控制 SCB (System Control Block) 的 SYSRESETREQ** : 使異常時能自動恢復與軟體重置
    ...
    participant FLASH as Flash 記憶體 (LMA)
    participant RAM as SRAM 記憶體 (VMA)
    participant APP as 使用者程式 (main)

    HW->>HW: 加載 Vector Table[0] (MSP)
    HW->>HW: 跳轉至 Vector Table[1] (Reset_Handler)
    
    Note over HW, RAM: --- 初始化階段 ---
    loop 複製 .data
        FLASH->>HW: 讀取初始值 (_etext)
        HW->>RAM: 寫入變數空間 (_sdata)
    end

    loop 清空 .bss
        HW->>RAM: 將位址空間設為 0 (_sbss to _ebss)
    end

    Note over HW, APP: --- 執行階段 ---
    HW->>APP: 呼叫 main()
    
    alt main 異常退出
        APP->>HW: 返回 Reset_Handler
        HW->>HW: 寫入 0xE000ED0C (Soft Reset)
        HW->>HW: 系統重新啟動
    end
    ```
